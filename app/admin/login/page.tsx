  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/auth/admin-login', {
        ...
        // robust: include cookies (safe for same-origin; helps in some deployments)
        credentials: 'include',
        body: JSON.stringify({ username, password }),
      });

      // Robust JSON parse (evită crash dacă răspunsul nu e JSON)
      let data: any = null;
      try {
      ...
      if (response.ok) {
        // cookie set pe response -> refresh + redirect
        router.refresh();
        router.push('/admin');
        setIsLoading(false);
        return;
